...
CachePlugin.prototype.apply = function(compiler) {
	if(Array.isArray(compiler.compilers)) {
		...
		compiler.plugin("run", function(compiler, callback) {
			...
			if (compiler._lastFileHashes) {
				compiler.fileHashes = compiler._lastFileHashes;
			}
		...
		compiler.plugin("after-compile", function(compilation, callback) {
			...
			compilation.compiler._lastFileHashes = compilation.compiler._lastFileHashes || {};
			async.forEach(compilation.modules, function(module, callback) {
				if (module.built && module._resourcePath) {
					var resourcePath = module._resourcePath();

					if (resourcePath) {
						hashForFile(resourcePath, compiler.inputFileSystem, function(err, hash) {
							if (err) return callback(err);
							compilation.compiler._lastFileHashes[resourcePath] = hash;
							callback();
						});
					} else {
						callback();
...
function hashForFile(resourcePath, filesystem, callback) {
	var hash = require("crypto").createHash("md5");
	filesystem.readFile(resourcePath, function(err, source) {
		if (err) {
			console.error('Error when reading', resourcePath)
			callback(err);
		}
