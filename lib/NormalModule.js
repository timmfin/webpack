...
NormalModule.prototype.needRebuildAsync = function needRebuildAsync(fileTimestamps, contextTimestamps, fs, fileHashes, callback) {
	var timestamp = 0;
	...
	if (timestamp >= this.buildTimestamp) {
		...
		if (!this._resourcePath || !(resourcePath = this._resourcePath())) {
			...
			callback(null, true);
		} else if (!fileHashes[resourcePath]) {
			...
			async.some(this.fileDependencies, function(file, innerCallback) {
				fs.readFile(file, function(err, content) {
					...
					innerCallback(fileHashes[file] !== newHash);
				}.bind(this));
