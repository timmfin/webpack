...
NormalModule.prototype.needRebuild = function needRebuild(fileTimestamps, contextTimestamps, fs, moduleHashes) {
	var timestamp = 0;
	...
	if (timestamp >= this.buildTimestamp) {
		if (!this.resource || !moduleHashes[this.resource]) {
			...
			var anyHashMismatches = async.some(this.fileDependencies, function(file, callback) {
				fs.readFile(file, function(err, content) {
					if (err) return callback(err);

					var newHash = this.getHashFor(content);
					callback(null, moduleHashes[file] !== newHash);
				}.bind(this));

			}.bind(this), function(err, result) {
				// ARG needs VVV but turned out to need to async
			});

			// return !anyHashMismatches;
			return true;  // TEMP IGNORE FOR NOW
		}
	} else {
		return false;
	}
