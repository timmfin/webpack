...
NormalModule.prototype.build = function build(options, compilation, resolver, fs, callback) {
	...
	return this.doBuild(options, compilation, resolver, fs, function(err) {
		...
		if(this._mtime) {
			this.buildTimestamp = this._mtime;
		} else {
...
NormalModule.prototype.needRebuildAsync = function needRebuildAsync(fileTimestamps, contextTimestamps, fs, moduleHashes, callback) {
	var timestamp = 0;
	...
	// First check for a new timestamp (of the file or any dependencies)
	// Then check if the module's hash has changed
	if (timestamp >= this.buildTimestamp) {
		var resourcePath;

		if (!this._resourcePath || !(resourcePath = this._resourcePath())) {
			// If this module doesn't refer to a file on the filesystem
			callback(null, true);
		} else if (!moduleHashes[resourcePath]) {
			// If there is no prior hash stored for this file
			callback(null, true);
		} else {

			async.some(this.fileDependencies, function(file, innerCallback) {
				fs.readFile(file, function(err, content) {
					// In async@1.5.x, there is no way to propogate the error in `some()`,
					// so ignore the fs error
					if (err) {
						console.error('Error reading', file, err);
						return innerCallback(true);
					}

					var newHash = this.getHashFor(content);

					// Doing a negative equality check here so some bails early on the first
					// file whose hash doesn't match
					innerCallback(moduleHashes[file] !== newHash);
				}.bind(this));

			}.bind(this), function(anyHashMismatches) {
				callback(null, anyHashMismatches);
			}.bind(this));
		}
	} else {
		callback(null, false);
...
NormalModule.prototype.getHashFor = function(string) {
	if(!string) return "";
	var hash = require("crypto").createHash("md5");
	hash.update(string);
	console.log('making hash for', this.request.split('/').slice(-1)[0], 'from content:', string);
	return hash.digest("hex");
}
