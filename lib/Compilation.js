...
Compilation.prototype.addModule = function(module, cacheGroup, callback) {
	// Allow cacheGroup to be optional
	if (callback === undefined && typeof cacheGroup === 'function') {
		callback = cacheGroup;
		cacheGroup = undefined;
	}
	cacheGroup = !cacheGroup ? "m" : cacheGroup;
	...
	if(this.cache && this.cache[cacheGroup + identifier]) {
		...
		var continueAddModule = function continueAddModule(rebuild) {
			this._rebuildIfNeeded(rebuild, module, cacheModule, function(err, cacheModule) {
				if (err) return callback(err);

				// If no rebuild was required, skip adding the module
				if (cacheModule) {
					callback(null, cacheModule);
				} else {
					this._finishAddingModule(module, cacheGroup, callback);
				}
			}.bind(this))
		}.bind(this);

		if(!cacheModule.error && cacheModule.cacheable && this.fileTimestamps && this.contextTimestamps && this.moduleHashes) {
			if (cacheModule.needRebuildAsync) {
				cacheModule.needRebuildAsync(this.fileTimestamps, this.contextTimestamps, this.inputFileSystem, this.moduleHashes, function(err, rebuild) {
					if (err) return callback(err);
					continueAddModule(rebuild);
				}.bind(this));
			} else {
				var rebuild = cacheModule.needRebuild(this.fileTimestamps, this.contextTimestamps, this.inputFileSystem, this.moduleHashes);
				continueAddModule(rebuild);
			}
		}
		else {

			// Always rebuild if there wasn't a cache to check
			continueAddModule(true);
		}
...
Compilation.prototype._rebuildIfNeeded = function(rebuild, module, cacheModule, callback) {
	if(!rebuild) {
...
Compilation.prototype._finishAddingModule = function(module, cacheGroup, callback) {
	var identifier = module.identifier();
...
Compilation.prototype.addModuleDependencies = function(module, dependencies, bail, cacheGroup, recursive, callback) {
	...
	async.forEach(factories, function(item, callback) {
		...
		factory.create(module.context, dependencies[0], function(err, dependantModule) {
			...
			_this.addModule(dependantModule, cacheGroup, function(err, newModule) {
...
Compilation.prototype._addModuleChain = function process(context, dependency, onModule, callback) {
	...
	moduleFactory.create(context, dependency, function(err, module) {
		...
		this.addModule(module, function(err, result) {
