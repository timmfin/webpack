...
Compilation.prototype.addModule = function(module, cacheGroup, callback) {
	...
	if(this.cache && this.cache[cacheGroup + identifier]) {
		...
		if(!cacheModule.error && cacheModule.cacheable && this.fileTimestamps && this.contextTimestamps && this.fileHashes) {
			if (cacheModule.needRebuildAsync) {
				cacheModule.needRebuildAsync(this.fileTimestamps, this.contextTimestamps, this.inputFileSystem, this.fileHashes, function(err, rebuild) {
					if (err) return callback(err);
					continueAddModule(rebuild);
				}.bind(this));
			} else {
				var rebuild = cacheModule.needRebuild(this.fileTimestamps, this.contextTimestamps, this.inputFileSystem, this.fileHashes);
				continueAddModule(rebuild);
			}
		}
		else {
